...
### 5. 반기가 넘어가도 대상을 잃지 않게

하반기 대시보드는 **상반기에 무중단이었던 대상(173대)**만 집계한다 — 상반기에 실전환이었던 대상(102대)은 하반기 무중단 훈련 목표가 아니기 때문에 집계에서 빠진다. 문제는 담당자가 하반기 시트에 수행방식을 다시 기재하면서(예: 무중단 → 실전환으로 바뀜) 이 173대 안에서도 수행방식이 섞이고, 가려진 102대도 여전히 일정·증적을 입력하고 싶어 한다는 데 있었다.

**통계(완료율·팀별·리포트)는 173대 기준을 그대로 유지하고, 입력 테이블에만 102대를 참고용으로 이어붙였다.** 집계 함수(`calc_completion`)를 173대와 102대에 각각 따로 돌려 상단 숫자에는 놓이지 않고 details만 붙이는 식이라, 이미 나간 진척률 발송분과 숫자가 어긋나지 않는다. 화면에는 "173 / 275대"로 둘 다 보여준다 (173 + 102 = 275, 상반기 전체 대상 수와 일치).
