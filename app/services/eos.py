...
def build_eos_ticket_summary(issues: list[dict], field_id: str) -> list[dict]:
    """JIRA 원본 -> 필요 필드만 (kind 판별만 IP전환/생성 기준으로 다름)"""
    return build_ticket_summary(issues, field_id, kind_fn=eos_ticket_kind)
...
def judge_eos(
    ...
    (2) 매칭된 [예방1] 티켓 중 "IP전환" 작업의 변경계획시작일이 하반기 창 안 + 기준일 이전인 경우만 완료.
    """
...
def calc_eos_completion(
    ...
    for item in targets:
        ...
        in_window = [
            t for t in matched
            if (dd := eos_ticket_done_date(t)) and w_start <= dd <= w_end
        ]
