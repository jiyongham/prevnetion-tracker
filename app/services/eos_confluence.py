...
- 각 페이지 안 표의 '작업 계획' 컬럼(4번째 셀)에 "[예방1][회사구분] 시스템명... IP전환 작업 (날짜)"
  형태로 EoS 작업이 한 줄씩 적혀 있다.
"""
import difflib
import io
import re
...
# 둘 다 같은 작업을 가리키는 텍스트라 공백/날짜 표기(예: '8/27' vs '827')만 다른 수준으로
# 거의 일치하므로, 이 정도로 높게 잡아도 실제 매칭엔 문제없고 엉뚱한 첨부는 걸러진다.
...
# 엉뚱한 시스템끼리 오매칭된다 (예: '전자저울 개발' 작업이 '온라인몰 개발 DB'로 잘못 매칭됨).
# 이 프로젝트 EoS 대상 중 이 값 이하로만 등장하는(=이름을 특정할 수 있는) 토큰만 매칭에 쓴다.
...
def _row_tokens(text: str) -> set[str]:
    """
    작업계획 텍스트 -> 매칭용 토큰 집합. '[예방1]'/'[회사명]' 같은 대괄호 태그는 전부 제거하고
    (그대로 두면 회사명 한 단어만으로 같은 회사 다른 시스템끼리 오매칭됨) 공백/구두점으로 분리.
    부분 문자열이 아니라 정확히 일치하는 토큰만 매칭 근거로 쓰기 위해 item 토큰과 세트로 비교한다
    (안 그러면 '시스템'이 '인시생산성시스템' 안에 우연히 포함돼 오매칭됨).
    """
    text = re.sub(r"\[[^\]]*\]", " ", text)
    text = text.replace("■", " ")
