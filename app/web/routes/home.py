...
그대로 재사용한다 (DR/EoS/용량관리 모두 각자의 캐시를 타 비용이 거의 없다 - 용량관리도
한때는 매 요청 JIRA를 직접 불러 포털 홈처럼 DATA/ARCH 두 시트를 순서대로 훑는 화면에서
매 방문 최대 2번 블로킹됐지만, capacity_data 캐시가 생기면서 해소됐다). 외부 조회가
실패해도 카드 하나가 "확인 필요"로 빠질 뿐 포털 진입 자체는 항상 뜨게 한다.
"""
import logging
