...
def dashboard(
    ...
    # 대상 여부(O,X)에서 X로 제외된 건수. Teams 리포트 "1. 전체 대상"과 같은
    # 기준으로, 그 반기 엑셀 전체를 기준으로 센다 (H2도 이제 275대 전체가 대상이라
    # load_dr_items_merged 직접 결과와 dr_data.load_items 결과의 대상 수가 같다).
